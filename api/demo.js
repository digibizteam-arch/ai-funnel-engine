...
function buildContentPrompt(roleType, data) {
  ...
  const frameworks = {
    coach: `You are writing for a COACH or TRAINER. Framework: Authority + Transformation.
Psychology: Speak to identity shift. The reader wants to BECOME someone. Use "you deserve", "you were meant for", "it's not your fault". Lead with the gap between who they are and who they could be. Make them feel understood first, then inspired.
Headline formula: "[Pain they feel] — [Promise of who they become]"
Testimonials must include: specific transformation result + emotional before/after.`,

    advisor: `You are writing for a FINANCIAL ADVISOR or INSURANCE AGENT. Framework: Trust + Protection.
Psychology: Speak to fear of loss, family security, and missed opportunity. Use calm authority. Never hype. Lead with what's at risk if they don't act. Use words like "protect", "secure", "peace of mind", "your family deserves".
Headline formula: "[What they risk losing] — [How you protect it]"
Testimonials must include: specific financial result + family/security angle.`,

    agency: `You are writing for an AGENCY or FREELANCER. Framework: Results + Speed.
Psychology: Speak to ROI and done-for-you appeal. Clients are tired and busy. They want speed and certainty. Use numbers, timelines, before/after metrics. Lead with the cost of NOT having a system.
Headline formula: "[Time wasted / money lost] — [Specific result in X days]"
Testimonials must include: specific metric improvement + time saved.`,

    local: `You are writing for a LOCAL BUSINESS OWNER. Framework: Community + Social Proof.
Psychology: Local trust is everything. Use neighborhood language. People buy from people they know. Emphasize "your area", "clients like you", "trusted by [type of client] in [region]". Make it feel familiar and personal.
Headline formula: "[Local frustration] — [Local solution with trust signal]"
Testimonials must include: local context + word-of-mouth feel.`,

    sales: `You are writing for a SALES PROFESSIONAL. Framework: Pipeline + Income.
Psychology: Speak to their income ceiling and pipeline anxiety. They track numbers. Use pipeline language: "qualified leads", "closing rate", "monthly target". Make it feel like a system upgrade, not a marketing tool.
Headline formula: "[Pipeline problem] — [Income/conversion outcome]"
Testimonials must include: pipeline metric + income improvement.`,

    seller: `You are writing for an ONLINE SELLER. Framework: Desire + Aspiration.
Psychology: Speak to lifestyle and freedom. Readers want to escape the grind. Use aspirational language: "wake up to orders", "work from anywhere", "your store works while you sleep". Lead with the dream, then the path.
Headline formula: "[Current hustle struggle] — [Freedom/lifestyle outcome]"
Testimonials must include: income result + lifestyle freedom element.`,

    consultant: `You are writing for a CONSULTANT or STRATEGIC ADVISOR. Framework: Credibility + Process.
Psychology: Speak to expertise and structured thinking. Clients want someone who has done this before and has a system. Use process language: "proven framework", "step-by-step", "based on X cases". Lead with authority, then clarity.
Headline formula: "[Complexity/confusion they face] — [Your structured solution]"
Testimonials must include: business outcome + clarity/confidence gained.`,

    default: `You are writing for a BUSINESS OWNER. Framework: Problem-Solution-Result.
Psychology: Speak to frustration with the current situation and desire for a clear path forward. Use empathetic, direct language. Lead with their specific pain, then paint the outcome.
Headline formula: "[Specific pain] — [Specific outcome]"
Testimonials must include: specific result + emotional relief.`
  };
      ...
      <div style="display:flex;align-items:center;gap:56px;flex-wrap:wrap;justify-content:center">
        <div style="flex-shrink:0;text-align:center">
          ...
          <div style="display:flex;gap:8px;justify-content:center;flex-wrap:wrap">
            ${['Forbes','LinkedIn','Entrepreneur'].map(b=>`<span style="background:${col.accent}15;border:1px solid ${col.accent}33;color:${col.accent};font-size:10px;font-weight:600;padding:4px 12px;border-radius:100px">${b}</span>`).join('')}
          </div>
        </div>
        <div style="flex:1;min-width:280px;max-width:560px">
          ...
          <div style="display:flex;flex-direction:column;gap:12px">
            ${(c.about_credentials||[]).map(cr=>`
            <div style="display:flex;align-items:flex-start;gap:12px">
              <div style="width:22px;height:22px;border-radius:50%;background:${col.accent};color:${col.btnText};display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;flex-shrink:0;margin-top:1px">✓</div>
              <span style="color:${col.text};font-size:0.93rem;line-height:1.55">${cr}</span>
            </div>`).join('')}
          </div>
        </div>
      </div>
...
body{font-family:'DM Sans',sans-serif;background:${col.bg};color:${col.text};line-height:1.6;overflow-x:hidden}
a{color:inherit;text-decoration:none}
.container{max-width:1060px;margin:0 auto;padding:0 24px}
.btn-primary{display:inline-block;background:${col.accent};color:${col.btnText};font-family:'DM Sans',sans-serif;font-size:1rem;font-weight:700;padding:17px 40px;border-radius:100px;border:none;cursor:pointer;transition:all 0.2s;text-decoration:none;letter-spacing:0.01em}
.btn-primary:hover{background:${col.accentDark};transform:translateY(-2px);box-shadow:0 12px 32px ${col.accent}44}
.eyebrow{display:inline-block;background:${col.accent}18;border:1px solid ${col.accent}44;color:${col.accent};font-size:11px;font-weight:600;letter-spacing:0.15em;text-transform:uppercase;padding:6px 18px;border-radius:100px;margin-bottom:20px}
.section-title{font-family:'Playfair Display',serif;font-size:clamp(1.8rem,3.5vw,2.8rem);line-height:1.2;margin-bottom:14px}
...
<!-- NAV -->
<nav style="position:sticky;top:0;z-index:100;background:${col.navBg};backdrop-filter:blur(14px);border-bottom:1px solid ${col.border};padding:14px 0">
...
<section style="padding:110px 0 90px;background:${col.bg};position:relative;overflow:hidden">
  ...
  <div class="container" style="text-align:center;position:relative">
    ...
    <div style="display:flex;align-items:center;justify-content:center;gap:0;flex-wrap:wrap">
      ${(c.hero_trust||'Proven Framework · Real Results · No Lock-in').split('·').map((t,i,arr)=>`
      <span style="font-size:0.8rem;color:${col.textMid};opacity:0.7;padding:0 16px;${i<arr.length-1?'border-right:1px solid '+col.border:''}">${t.trim()}</span>`).join('')}
    </div>
  </div>
</section>
...
<section style="padding:80px 0;background:${col.bg2}">
  <div class="container">
    ...
    <div class="grid-4" style="display:grid;grid-template-columns:repeat(4,1fr);gap:18px;margin-bottom:44px">
      ${(c.pain_bullets||[]).map(b=>`
      <div style="background:${col.cardBg};border:1px solid ${col.border};border-top:3px solid ${col.accent};border-radius:14px;padding:24px 20px">
        <div style="font-size:2rem;margin-bottom:14px">${b.emoji}</div>
        <div style="font-weight:700;font-size:0.95rem;color:${col.text};margin-bottom:8px;line-height:1.3">${b.bold}</div>
        <div style="font-size:0.83rem;color:${col.textMid};line-height:1.6">${b.detail}</div>
      </div>`).join('')}
    </div>
    <div style="text-align:center">
      <div style="display:inline-block;background:${col.accent}12;border:1px solid ${col.accent}33;border-radius:14px;padding:20px 36px;font-size:1.02rem;font-style:italic;color:${col.text};max-width:620px">
        "${c.pain_closer||''}"
      </div>
    </div>
  </div>
...
<section style="padding:80px 0;background:${col.bg3}">
  <div class="container">
    <div style="text-align:center;margin-bottom:48px">
      ...
      <h2 class="section-title">Two paths. One decision.</h2>
    </div>
    <div class="grid-2" style="display:grid;grid-template-columns:1fr 1fr;gap:0;border-radius:20px;overflow:hidden;border:1px solid ${col.border}">
      <div style="padding:40px 32px;background:${col.cardBg}">
        <div style="font-size:0.72rem;font-weight:700;letter-spacing:0.15em;color:${col.textMid};margin-bottom:24px;text-transform:uppercase">${c.gap_left_title||'WHERE YOU ARE NOW'}</div>
        <div style="display:flex;flex-direction:column;gap:16px">
          ${(c.gap_left||[]).map(b=>`
          <div style="display:flex;align-items:flex-start;gap:12px">
            <span style="color:#ef4444;flex-shrink:0;font-weight:700;margin-top:1px;font-size:1rem">✗</span>
            <span style="color:${col.textMid};font-size:0.92rem;line-height:1.55">${b}</span>
          </div>`).join('')}
        </div>
      </div>
      <div style="padding:40px 32px;background:${col.accent}12;border-left:3px solid ${col.accent}">
        <div style="font-size:0.72rem;font-weight:700;letter-spacing:0.15em;color:${col.accent};margin-bottom:24px;text-transform:uppercase">${c.gap_right_title||'WHERE YOU COULD BE'}</div>
        <div style="display:flex;flex-direction:column;gap:16px">
          ${(c.gap_right||[]).map(b=>`
          <div style="display:flex;align-items:flex-start;gap:12px">
            <span style="color:${col.accent};flex-shrink:0;font-weight:700;margin-top:1px;font-size:1rem">✓</span>
            <span style="color:${col.text};font-size:0.92rem;line-height:1.55;font-weight:500">${b}</span>
          </div>`).join('')}
        </div>
      </div>
    </div>
    <div style="text-align:center;margin-top:32px">
      <p style="font-size:1.05rem;color:${col.textMid};font-style:italic">${c.gap_bridge||''}</p>
    </div>
  </div>
</section>
...
<section style="padding:56px 0;background:${col.accent};overflow:hidden">
  <div class="container">
    <div class="grid-3" style="display:grid;grid-template-columns:repeat(3,1fr);gap:0;text-align:center">
      ${(c.stats||[]).map((s,i,arr)=>`
      <div style="padding:24px;${i<arr.length-1?'border-right:1px solid '+col.btnText+'33':''}">
        <div style="font-family:'Playfair Display',serif;font-size:2.8rem;font-weight:700;color:${col.btnText};line-height:1">${s.number}</div>
        <div style="font-size:0.82rem;color:${col.btnText};opacity:0.75;margin-top:8px;font-weight:500">${s.label}</div>
      </div>`).join('')}
    </div>
  </div>
</section>
...
<section id="offer" style="padding:90px 0;background:${col.bg2}">
  <div class="container">
    <div style="text-align:center;margin-bottom:52px">
      <div class="eyebrow">${c.offer_eyebrow||'INTRODUCING'}</div>
      <h2 class="section-title" style="max-width:720px;margin:0 auto 14px">${c.offer_headline||''}</h2>
      <p style="color:${col.textMid};font-size:1rem;max-width:540px;margin:0 auto">${c.offer_sub||''}</p>
    </div>
    <div class="grid-2" style="display:grid;grid-template-columns:1fr 1fr;gap:18px;margin-bottom:44px">
      ${(c.offer_benefits||[]).map(b=>`
      <div style="background:${col.cardBg};border:1px solid ${col.border};border-radius:16px;padding:26px;display:flex;align-items:flex-start;gap:16px;transition:transform 0.2s" onmouseover="this.style.transform='translateY(-3px)'" onmouseout="this.style.transform='translateY(0)'">
        <div style="width:38px;height:38px;border-radius:50%;background:${col.accent};color:${col.btnText};display:flex;align-items:center;justify-content:center;font-size:1rem;font-weight:700;flex-shrink:0">${b.icon}</div>
        <div>
          <div style="font-weight:700;font-size:0.97rem;color:${col.text};margin-bottom:6px">${b.title}</div>
          <div style="font-size:0.85rem;color:${col.textMid};line-height:1.6">${b.desc}</div>
        </div>
      </div>`).join('')}
    </div>
    <!-- Price card -->
    <div style="background:linear-gradient(135deg,${col.accent},${col.accentDark});border-radius:20px;padding:48px;text-align:center;max-width:520px;margin:0 auto;box-shadow:0 20px 60px ${col.accent}33">
      <div style="font-size:0.78rem;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:${col.btnText};opacity:0.8;margin-bottom:12px">Investment</div>
      <div style="font-family:'Playfair Display',serif;font-size:3rem;font-weight:700;color:${col.btnText};margin-bottom:8px;line-height:1">${c.offer_price_display||'Contact for Pricing'}</div>
      <div style="font-size:0.88rem;color:${col.btnText};opacity:0.72;margin-bottom:32px;line-height:1.5">${c.offer_price_context||''}</div>
      <a href="${chatUrl}" style="display:inline-block;background:${col.btnText};color:${col.accent};font-family:'DM Sans',sans-serif;font-size:1rem;font-weight:700;padding:16px 40px;border-radius:100px;text-decoration:none;transition:all 0.2s" onmouseover="this.style.opacity='0.9'" onmouseout="this.style.opacity='1'">${c.offer_cta||'Get Started →'}</a>
    </div>
  </div>
</section>
...
<section style="padding:80px 0;background:${col.bg}">
  <div class="container">
    <div style="text-align:center;margin-bottom:52px">
      ...
      <h2 class="section-title">${c.process_title||'How It Works'}</h2>
    </div>
    <div class="grid-3" style="display:grid;grid-template-columns:repeat(3,1fr);gap:24px;position:relative">
      ...
      <div style="text-align:center;padding:32px 24px;position:relative">
        <div style="width:56px;height:56px;border-radius:50%;background:${col.accent};color:${col.btnText};display:flex;align-items:center;justify-content:center;font-family:'Playfair Display',serif;font-size:1.2rem;font-weight:700;margin:0 auto 20px">${s.num}</div>
        ${i<arr.length-1?`<div style="position:absolute;top:56px;right:-12px;width:24px;height:2px;background:${col.accent};opacity:0.4;display:none"></div>`:''}
        <div style="font-weight:700;font-size:1rem;color:${col.text};margin-bottom:10px">${s.title}</div>
        <div style="font-size:0.85rem;color:${col.textMid};line-height:1.65">${s.desc}</div>
      </div>`).join('')}
    </div>
  </div>
</section>
...
<section style="padding:80px 0;background:${col.bg2}">
  <div class="container">
    <div style="text-align:center;margin-bottom:52px">
      <div class="eyebrow">WHAT CLIENTS ARE SAYING</div>
    ...
    <div class="grid-3" style="display:grid;grid-template-columns:repeat(3,1fr);gap:20px">
      ...
      <div style="background:${col.cardBg};border:1px solid ${col.border};border-radius:18px;padding:28px;display:flex;flex-direction:column;gap:18px">
        <div style="color:${col.accent};font-size:1rem;letter-spacing:2px">★★★★★</div>
        <p style="font-size:0.9rem;color:${col.textMid};line-height:1.75;flex:1;font-style:italic">"${t.quote}"</p>
        <div style="background:${col.accent}15;border-radius:8px;padding:8px 12px;font-size:0.78rem;font-weight:600;color:${col.accent}">${t.result}</div>
        <div style="display:flex;align-items:center;gap:12px;padding-top:14px;border-top:1px solid ${col.border}">
          <div style="width:40px;height:40px;border-radius:50%;background:linear-gradient(135deg,${col.accent},${col.accentDark});display:flex;align-items:center;justify-content:center;font-size:0.9rem;font-weight:700;color:${col.btnText};flex-shrink:0">${t.initial||t.name.charAt(0)}</div>
          <div>
            <div style="font-weight:600;font-size:0.88rem;color:${col.text}">${t.name}</div>
            <div style="font-size:0.75rem;color:${col.textMid}">${t.role}</div>
          </div>
        </div>
      </div>`).join('')}
    </div>
...
<section style="padding:80px 0;background:${col.bg}">
  <div class="container" style="max-width:720px">
    ...
    <div style="display:flex;flex-direction:column;gap:14px">
      ${(c.faq||[]).map(f=>`
      <div style="background:${col.cardBg};border:1px solid ${col.border};border-radius:14px;padding:24px 28px">
        <div style="font-weight:700;font-size:0.97rem;color:${col.text};margin-bottom:10px">Q: ${f.q}</div>
        <div style="font-size:0.88rem;color:${col.textMid};line-height:1.7">${f.a}</div>
      </div>`).join('')}
    </div>
  </div>
</section>
...
<!-- FINAL CTA -->
<section style="padding:90px 0;background:${col.accent};position:relative;overflow:hidden">
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    }, { model: 'gpt-4o-mini', messages: [{ role: 'user', content: prompt }], temperature: 0.8, max_tokens: 2500 });
  ...
  try {
    ...
    if (lead_id) {
      await postJSON({
        hostname: supabaseURL.hostname, path: `/rest/v1/leads?id=eq.${lead_id}`, method: 'PATCH',
