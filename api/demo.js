...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0} html{scroll-behavior:smooth} body{font-family:'DM Sans',sans-serif;background:${c.bg};color:${c.text};line-height:1.6} a{color:inherit;text-decoration:none} img{max-width:100%;display:block} .container{max-width:1060px;margin:0 auto;padding:0 24px} .btn-primary{display:inline-block;background:${c.accent};color:${c.btnText};font-family:'DM Sans',sans-serif;font-size:1rem;font-weight:600;padding:16px 36px;border-radius:100px;border:none;cursor:pointer;transition:all 0.2s;text-decoration:none;letter-spacing:0.01em} .btn-primary:hover{background:${c.accentDark};transform:translateY(-2px);box-shadow:0 8px 24px ${c.accent}44} .btn-outline{display:inline-block;background:transparent;color:${c.accent};font-family:'DM Sans',sans-serif;font-size:0.95rem;font-weight:500;padding:14px 32px;border-radius:100px;border:1.5px solid ${c.accent};cursor:pointer;transition:all 0.2s;text-decoration:none} .btn-outline:hover{background:${c.accent};color:${c.btnText}} .section-label{display:inline-block;background:${c.accent}18;border:1px solid ${c.accent}44;color:${c.accent};font-size:11px;font-weight:600;letter-spacing:0.15em;text-transform:uppercase;padding:6px 18px;border-radius:100px;margin-bottom:20px} .card{background:${c.cardBg};border:1px solid ${c.border};border-radius:16px;padding:28px 24px}
    @media(max-width:640px){ .container{padding:0 16px} .btn-primary{padding:14px 28px;font-size:0.92rem} h1{font-size:2rem!important} .grid-2{grid-template-columns:1fr!important} .grid-3{grid-template-columns:1fr!important} .grid-4{grid-template-columns:1fr 1fr!important} section{padding:56px 0!important} }
  </style>
</head>
<body>
  <!-- NAV -->
  ...
  try {
    ...
    if (lead_id) {
      ...
      // FALLBACK: Create a new lead if analyze.js failed
      const inserted = await postJSON({
        ...
        name,
        email,
        role: role || 'Not provided',         // <--- FIX: Save quiz data even on fallback
        goal: goal || 'Not provided',         // <--- FIX: Save quiz data even on fallback
        problem: problem || 'Not provided',   // <--- FIX: Save quiz data even on fallback
        offer_name:offer,
        offer_price:price||null,
        ideal_client:client,
        offer_pain:pain,
